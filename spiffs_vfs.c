...
static off_t vfs_lseek(void *ctx, int fd, off_t offset, int whence) {
	...
	spiffs *fs = (spiffs *)ctx;
	int rc = SPIFFS_lseek(fs, (spiffs_file)fd, offset, whence);
	return rc;
} // vfs_lseek
...
static int vfs_open(void *ctx, const char *path, int flags, int accessMode) {
	if (path[0] == '/') path++; // remove leading slash
	...
	ESP_LOGI(tag, ">> SPIFFS_open path=%s, spiffsFlags=0x%x, accessMode=0x%x", path , spiffsFlags, accessMode);
...
static int vfs_fstat(void *ctx, int fd, struct stat *st) {
	...
	spiffs *fs = (spiffs *)ctx;
        spiffs_stat sst; /* obj_id, size, type, pix, name */
	int rc = SPIFFS_fstat(fs, (spiffs_file)fd, &sst);
        st->st_ino = sst.obj_id;
        st->st_size = sst.size;
	return rc;
} // vfs_fstat
...
static int vfs_stat(void *ctx, const char *path, struct stat *st) {
	...
	spiffs *fs = (spiffs *)ctx;
        spiffs_stat sst; /* obj_id, size, type, pix, name */
	int rc = SPIFFS_stat(fs, path, &sst);
        st->st_ino = sst.obj_id;
        st->st_size = sst.size;
	return rc;
} // vfs_stat
...
static int vfs_link(void *ctx, const char *oldPath, const char *newPath) {
	if (oldPath[0] == '/') oldPath++; // remove leading slash
	if (newPath[0] == '/') newPath++; // remove leading slash
	ESP_LOGI(tag, ">> UNIMPLEMENTED link oldPath=%s, newPath=%s", oldPath, newPath);
	return 0;
} // vfs_link
